...
- Keep `agent/extensions/workflow-hooks.ts` limited to Pi event translation; shared behavior belongs in the `../tools/workflow-hooks/` Rust binary.
...
- Keep credentials and Pi runtime state outside tracked files. Under `agent/`, `../.gitignore` whitelists only `extensions/`, so a new tracked path there needs its own whitelist entry.
