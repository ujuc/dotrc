...
10항의 적용 예로, 원문이 시제를 명시하지 않은 사건 명사구에 시제를 넣는 변경은 위반이다. "일부 요청 지연 가능성"을 "일부 요청의 지연 가능성이 있습니다"로 완결하는 것은 허용하지만, "지연될 가능성"이나 "지연되었을 가능성"으로 바꾸면 롤백 대상이다. 원문에 없는 담당자·원인·범위·결과를 넣어 모호한 대상을 구체화한 변경도 13항 위반이다. 코드·식별자·명령어·경로·API 필드·로그·오류 메시지·커밋 메시지의 철자·대소문자·구두점 변경은 1항 위반으로 처리한다.
