...
### A-16. 개조식 불완전 종결 (명사구·부사구·연결어미 종결) [S2] · v1.6 신규
- 패턴: 설명문이 서술어와 종결어미 없이 명사구·부사구·연결어미로 끝남
- 예: "API 교체가 완료됐음. 기존 설정 파일에도 경고가 추가됐음." → "API 교체가 완료되었습니다. 기존 설정 파일에도 경고가 추가되었습니다."(원문의 `됐음`에 이미 과거 시제가 있어 종결만 보완한 경우)
- 예외: 제목·표 셀·레이블·짧은 상태 표시, 그리고 문서 전체가 의도적으로 개조식·전보식인 경우. 이때는 탐지만 기록하고 고치지 않는다.
- 처방: 서술어와 종결어미로 완결한다. **원문에 없는 시제를 넣지 않는다** — 시제가 없는 사건 명사구는 술어만 보완한다. "일부 요청 지연 가능성" → "일부 요청의 지연 가능성이 있습니다"(허용) / "지연될 가능성"·"지연되었을 가능성"(금지).
- 시제 단서가 없는 맨 사건 명사구("경고 추가.")는 문맥이 시제를 확정할 때만 완결한다. 확정할 수 없으면 고치지 않고 finding으로만 기록한다. 체크리스트의 "경고 추가."를 "경고가 추가되었습니다."로 바꾸면 예정 항목이 완료 사실로 바뀐다.
- 밀도 판단: 설명문 성격의 문장·목록 항목에서만 계산한다. 개조식 문서 여부는 `context_flags`에 남긴다.

### A-17. 필수 성분 생략 + 관형격 "~의" 과다 [S2] · v1.6 신규
- 패턴: 대상·조건·영향 범위가 빠져 지시가 모호함. 관형격 조사 "~의"를 겹쳐 쓰면서 의미를 담은 성분이 사라짐
- 예: "그러면 경고가 붙습니다"(무엇에 붙는지 없음) / "사본의 문구는 작업의 상황을" → "사본에 기재된 문구는 작업이 진행되는 상황을"
- 처방: **원문에서 확정할 수 있는 성분만 복원한다.** 담당자·원인·범위·결과를 추정해 만들어 넣지 않는다. 문맥으로 확정할 수 없으면 고치지 않고 finding으로만 기록한다.
...
- 처방: 기존 문장을 의미 단위로 분리·병합해 리듬을 조정한다. 새 단문·장문·논평을 삽입하지 않는다.
...
### E-3. 모든 문단 3~4문장 공식
- 문단 길이도 균일.
- 처방: 기존 문단 경계를 의미 단위로 분리·병합한다. 내용이나 강조를 새로 만들지 않는다.
...
아래는 원본 프로젝트의 역사 기록이다. `_workspace/`와 과거 `references/` 경로는
현재 배포에 포함되지 않으며 런타임 로딩 대상이 아니다. 현재 참조 목록은
[Humanizer의 References](../SKILL.md#references)를 따른다.

- **v1.6** (2026-09-01): **본진 신규 2건** — `A-16` 개조식 불완전 종결 [S2] · `A-17` 필수 성분 생략 + 관형격 "~의" 과다 [S2]. 출처는 [`snflkd/fluent-korean`](https://github.com/snflkd/fluent-korean)(MIT)의 한국어 명확성 지침이며, 원문의 "무조건 완결" 규범은 이 분류 체계의 밀도 기반 S2로 조정했다. 함께 반영: 보호 대상에 코드·식별자·명령어·경로·API 필드·로그·오류 메시지·커밋 메시지 추가, 시제 주입 금지 가드 명시.
